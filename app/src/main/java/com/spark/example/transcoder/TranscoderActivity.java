...
import com.spark.example.MainActivity;
...
public class TranscoderActivity extends AppCompatActivity implements
    ...
    private void syncParameters() {
        int channels;
        int c_id=mAudioChannelsGroup.getCheckedRadioButtonId();
        if (c_id==R.id.channels_mono) channels = 1;
        else if (c_id==R.id.channels_stereo) channels = 2;
        else channels = DefaultAudioStrategy.CHANNELS_AS_INPUT;

//        switch (mAudioChannelsGroup.getCheckedRadioButtonId()) {
//            case channels_mono: channels = 1; break;
//            case channels_stereo: channels = 2; break;
//            default: channels = DefaultAudioStrategy.CHANNELS_AS_INPUT;
//        }

        int sampleRate;
        int sr_id=mAudioSampleRateGroup.getCheckedRadioButtonId();
        if(sr_id==R.id.sampleRate_32) sampleRate = 32000;
        else if(sr_id==R.id.sampleRate_48) sampleRate = 48000;
        else sampleRate = DefaultAudioStrategy.SAMPLE_RATE_AS_INPUT;

//        switch (mAudioSampleRateGroup.getCheckedRadioButtonId()) {
//            case sampleRate_32: sampleRate = 32000; break;
//            case sampleRate_48: sampleRate = 48000; break;
//            default: sampleRate = DefaultAudioStrategy.SAMPLE_RATE_AS_INPUT;
//        }

        boolean removeAudio;
        int ra_id=mAudioReplaceGroup.getCheckedRadioButtonId();
        if (ra_id==R.id.replace_remove) removeAudio = true;
        else if (ra_id==R.id.replace_yes) removeAudio = false;
        else removeAudio = false;

//        switch (mAudioReplaceGroup.getCheckedRadioButtonId()) {
//            case replace_remove: removeAudio = true; break;
//            case replace_yes: removeAudio = false; break;
...
//        }
        ...
        int frames;
        int f_id=mVideoFramesGroup.getCheckedRadioButtonId();
        if (f_id==R.id.frames_24) frames = 24;
        else if (f_id==R.id.frames_30) frames = 30;
        else if (f_id==R.id.frames_60) frames = 60;
        else frames = DefaultVideoStrategy.DEFAULT_FRAME_RATE;


//        switch (mVideoFramesGroup.getCheckedRadioButtonId()) {
//            case frames_24: frames = 24; break;
//            case frames_30: frames = 30; break;
//            case frames_60: frames = 60; break;
//            default: frames = DefaultVideoStrategy.DEFAULT_FRAME_RATE;
//        }

        float fraction;
        int fr_id=mVideoResolutionGroup.getCheckedRadioButtonId();
        if (fr_id==R.id.resolution_half) fraction = 0.5F;
        else if (fr_id==R.id.resolution_third) fraction = 1F / 3F;
        else fraction = 1F;
...
//        }
        ...
        float aspectRatio;
        int ar_id=mVideoAspectGroup.getCheckedRadioButtonId();
        if (ar_id==R.id.aspect_169) aspectRatio = 16F / 9F;
        else if (ar_id==R.id.aspect_43) aspectRatio = 4F / 3F;
        else if (ar_id==R.id.aspect_square) aspectRatio = 1F;
        else aspectRatio = 0F;

//        switch (mVideoAspectGroup.getCheckedRadioButtonId()) {
//            case aspect_169: aspectRatio = 16F / 9F; break;
//            case aspect_43: aspectRatio = 4F / 3F; break;
//            case aspect_square: aspectRatio = 1F; break;
...
//        }
    ...
    private void transcode(@NonNull Uri... uris) {
        ...
        try {
            File outputDir = new File(getExternalFilesDir(null), "Output_Videos");
            //noinspection ResultOfMethodCallIgnored
            outputDir.mkdir();
            mTranscodeOutputFile = File.createTempFile("transcode_", ".mp4", outputDir);
...
//        }
        ...
        float speed;
        int s_id=mSpeedGroup.getCheckedRadioButtonId();
        if (s_id==R.id.speed_3x) speed = 3F;
        else if (s_id==R.id.speed_2x) speed = 2F;
        else speed = 1F;

//        switch (mSpeedGroup.getCheckedRadioButtonId()) {
//            case speed_05x: speed = 0.5F; break;
//            case speed_2x: speed = 2F; break;
//            default: speed = 1F;
//        }
    ...
    public void onTranscodeCompleted(int successCode) {
        if (successCode == Transcoder.SUCCESS_TRANSCODED) {
            ...
            finish();
//            startActivity(new Intent(TranscoderActivity.this, MainActivity.class));
